...
- `GET /api/me/rates/chart` — sparkline-list chart data for the caller's subscribed pairs; authenticated via Telegram WebApp initData HMAC (`X-Telegram-Init-Data` header only). Query params: `period` (integer days, must be one of `{7, 30, 90, 180, 360}`, default 7; any other value returns 400 with a PublicError body). `kind=LAST` (equity) sources render under the `equity` category with their own amber series (`#D98E04`), distinct from BID/ASK fiat pairs.
- `GET /api/me/rates/history` — paginated rate-collection events for the calling user's subscribed sources matching a canonical pair label; authenticated via Telegram WebApp initData HMAC (`X-Telegram-Init-Data` header only). Query params: `pair` (required, e.g. `USD/KZT`), `source_title` (optional, filters to one provider by its human-readable title, which is unique per provider; unknown value returns 200 with empty items), `page` (default 1), `limit` (default 20, max 100). Response row fields: `bid` / `bid_delta_pct` (BID-kind sources), `ask` / `ask_delta_pct` (ASK-kind sources), `last` / `last_delta_pct` (LAST-kind equity sources, e.g. AAPL/USD); all fields use `omitempty` so FX rows never emit `last`.
...
- `GET /api/me/weather/current` — latest stored Open-Meteo observation for each of the caller's subscribed cities, deduplicated by `location_id`. Returns `{"items":[{"location_id":"...","display_name":"...","timezone":"...","has_data":bool,...}]}`. Cities whose first collection has not yet completed return an item with `has_data:false` (not an error). Sunrise/sunset are pre-formatted as `"15:04"` in the city's IANA timezone by the server. Authenticated via `X-Telegram-Init-Data` header only.
...
- `POST /api/me/weather/cities` — create a new city weather subscription; body: `{"location_id":"...", "display_name":"...", "latitude":..., "longitude":..., "timezone":"...", "country":"...", "admin1":"...", "notify_kind":"morning_summary|alert_heat|alert_frost|alert_thunderstorm|rain_alert", "condition_value":"..."}`. Server re-validates `timezone` via `time.LoadLocation`, `latitude` ∈ [-90,90], `longitude` ∈ [-180,180], `notify_hour` ∈ [0,23] (defaults to 7 when absent), and `condition_value` per kind (heat/frost temperature ∈ [-100,100]°C, rain probability ∈ (0,100]%, thunderstorm ignores it, morning_summary clears it). Returns `{"id":"..."}` on 201. Authenticated via `X-Telegram-Init-Data` header only.
...
- `GET /api/public/rates/chart` — paginated system-wide sparkline-list chart; no authentication. Query params: `page` (default 1), `limit` (default 20, max 100), `period` (integer days, must be one of `{7, 30, 90, 180, 360}`, default 7; any other value returns 400 with a PublicError body). Like `/api/me/rates/chart`, equity (`kind=LAST`) pairs render under the `equity` category.
